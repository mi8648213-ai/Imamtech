...
async function loadSupabase(){
  ...
  supabaseClient.auth.onAuthStateChange((_event,session)=>{currentUser=session?.user||null; if(currentUser) enterApp();});
}
...
async function login(){
 ...
 const email=prompt('Enter your Imamtech account email:', 'mi8648213@gmail.com'); if(!email)return;
 ...
 toast('Check your email for the secure sign-in link.');
}
...
function enterApp(){$('#loginView').classList.add('hidden');$('#appView').classList.remove('hidden');loadData()}
